            ...
            >
              <div className="space-y-8 flex-grow">
                ...
                <div className="space-y-4">
                    <h2 className="text-3xl font-black text-text group-hover:text-accent transition-colors leading-tight tracking-tight uppercase">{blog.title}</h2>
                    ...
                    <div className="space-y-10 text-center max-w-4xl mx-auto relative">
                        ...
                        <div className="flex items-center justify-center gap-12 text-text/40 pt-10 border-t border-border/50 max-w-2xl mx-auto">
                            ...
                            <div className="text-left">
                                ...
                                <p className="font-black text-text/80 text-xl tracking-tight">{new Date(expandedBlog.created_at).toLocaleDateString()}</p>
                            </div>
                        </div>
                    </div>
            ...
            <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="bg-card border border-border rounded-[3.5rem] w-full max-w-5xl my-auto shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <form onSubmit={handleSubmit} className="p-10 md:p-14 space-y-10 overflow-y-auto">
                ...
                <div className="space-y-4">
                    ...
                    <input type="text" className="w-full bg-background border-2 border-border rounded-[1.5rem] py-6 px-10 focus:border-accent outline-none font-bold text-text shadow-inner transition-colors" value={formData.tags} onChange={(e) => setFormData({...formData, tags: e.target.value})} />
                </div>
                <button type="submit" className="bg-accent hover:bg-gfg-green-hover text-white font-black py-7 rounded-[1.5rem] transition text-xl shadow-2xl uppercase tracking-widest active:scale-[0.98]">Deploy Publication</button>
              </form>
            </motion.div>
          </div>
        )}
