...
const EventDetail = () => {
  ...
  const handleUnregister = async () => {
      const msg = teamStatus?.isLeader ? "Dissolve team and unregister?" : "Unregister from event?";
  ...
  const handleRemoveMember = async (memberId) => {
      if (window.confirm("Remove this member from your team?")) {
          try {
  ...
  if (loading) return <div className="py-24 text-center text-accent font-black tracking-widest uppercase animate-pulse text-xs italic">Loading...</div>;
            ...
            <div className="max-w-7xl mx-auto space-y-4">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap items-center gap-3">
                    <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest border backdrop-blur-md ${event.is_open ? 'bg-accent/80 text-white border-accent/20 shadow-lg' : 'bg-red-500/80 text-white border-red-500/20'}`}>
                        {event.is_open ? 'REGISTRATION OPEN' : 'REGISTRATION CLOSED'}
                    </span>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-card border border-border p-6 md:p-10 rounded-3xl shadow-xl space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-8 border-t border-border/50">
                        <div className="space-y-2">
                            <p className="text-text/20 text-[8px] font-black uppercase tracking-widest">Date & Time</p>
                        ...
                        <div className="space-y-2">
                            <p className="text-text/20 text-[8px] font-black uppercase tracking-widest">Location</p>
                            <div className="flex items-center gap-3 text-text/80 font-black uppercase tracking-widest text-[10px] bg-background border border-border p-4 rounded-xl shadow-inner">
                ...
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="bg-card border border-border p-8 rounded-3xl shadow-xl sticky top-20">
                    <div className="space-y-6">
                        <div className="text-center space-y-3">
                            ...
                            <h4 className="text-lg font-black text-text uppercase tracking-widest">Organizer</h4>
                            ...
                            <div className="space-y-6">
                                {teamStatus.isTeam ? (
                                    <div className="bg-accent/5 border border-accent/20 p-6 rounded-2xl space-y-4 shadow-inner">
                                        <div className="flex justify-between items-start">
                                            <div className="space-y-0.5">
                                                <p className="text-[8px] font-black text-accent uppercase tracking-widest">Your Team</p>
                                                {isEditingTeamName ? (
                                        ...
                                        <div className="space-y-2">
                                            <p className="text-[8px] font-black text-text/30 uppercase tracking-widest">Members ({teamStatus.members.length}/{event.max_team_size})</p>
                                        ...
                                        {teamStatus.isLeader && teamStatus.members.length < event.max_team_size && (
                                            <form onSubmit={handleInviteMember} className="space-y-2 pt-3 border-t border-border/30">
                                                <p className="text-[8px] font-black text-text/30 uppercase tracking-widest">Invite Member</p>
                                                <div className="flex gap-1.5">
                                                    <input required type="email" placeholder="member@email.com" className="flex-grow bg-background border border-border rounded-lg px-2 py-1.5 text-[10px] font-bold text-text outline-none focus:border-accent" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} />
                                    ...
                                    <div className="bg-accent/5 border border-accent/20 p-5 rounded-2xl text-center space-y-1.5 shadow-inner">
                                        ...
                                        <p className="text-[10px] font-black text-text uppercase tracking-widest">Registered</p>
                                        <p className="text-[8px] text-text/40 font-bold uppercase">Status: Confirmed</p>
                                    </div>
                                )}
                                ...
                                <button onClick={handleUnregister} className="w-full py-3.5 rounded-xl bg-red-500/5 text-red-500 border border-red-500/10 hover:bg-red-500 hover:text-white transition-all font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2 active:scale-95">
                                    <FontAwesomeIcon icon={faSignOutAlt} /> Unregister
                                </button>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {canRegister && (
                                    <button onClick={() => setIsRegistering(true)} disabled={!event.is_open} className={`w-full py-4 rounded-xl font-black text-sm uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-xl active:scale-95 ${event.is_open ? 'bg-accent hover:bg-gfg-green-hover text-white shadow-accent/20' : 'bg-background border border-border text-text/20 cursor-not-allowed'}`}>
                                        {event.is_open ? <><FontAwesomeIcon icon={faCheckCircle} /> Register Now</> : 'Registration Closed'}
                                    </button>
                                )}
                            </div>
                        )}
                        ...
                        {canManage && (
                            <div className="space-y-2 pt-3 border-t border-border/50">
                                ...
                                <button onClick={handleDelete} className="w-full py-3 rounded-lg bg-red-500/5 text-red-500 hover:bg-red-500 hover:text-white transition-all font-black uppercase text-[8px] tracking-widest flex items-center justify-center gap-2 border border-red-500/10 active:scale-95">
                                    <FontAwesomeIcon icon={faTrashAlt} /> Delete Event
                                </button>
                            </div>
                        )}
                    </div>
                ...
                <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="bg-card border border-border rounded-3xl w-full max-w-xl my-auto shadow-2xl overflow-hidden flex flex-col">
                    <div className="p-6 border-b border-border flex justify-between items-center bg-background/50">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-xl font-black text-text uppercase tracking-tight italic">Event Registration</h2>
                        </div>
                    ...
                    <form onSubmit={handleRegister} className="p-6 md:p-8 space-y-6">
                        <div className="space-y-3">
                            <label className="block text-[8px] font-black text-text/40 uppercase tracking-widest ml-1">Registration Type</label>
                        ...
                        {regType === 'team' && (
                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-4 overflow-hidden">
                                <div className="space-y-1.5">
                                    ...
                                    <input required type="text" className="w-full bg-background border-2 border-border rounded-xl py-3 px-4 focus:border-accent outline-none font-bold text-sm text-text shadow-inner italic" placeholder="Enter team name" value={teamName} onChange={(e) => setTeamName(e.target.value)} />
                                </div>
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center">
                                        <label className="block text-[8px] font-black text-text/40 uppercase tracking-widest ml-1">Member Emails</label>
                                        <span className="text-[7px] font-black text-accent uppercase tracking-widest">Max Members: {event.max_team_size - 1}</span>
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        {memberEmails.length < (event.max_team_size - 1) && (
                                            <button type="button" onClick={addMemberField} className="w-full py-2 border-2 border-dashed border-border rounded-xl text-text/30 hover:border-accent/50 hover:text-accent transition-all font-black text-[8px] uppercase tracking-widest">Add Member</button>
                                        )}
                                    </div>
                                </div>
                            </motion.div>
                        ...
                        <button type="submit" className="w-full bg-accent hover:bg-gfg-green-hover text-white font-black py-4 rounded-xl transition shadow-lg uppercase tracking-widest text-[10px] active:scale-95">Complete Registration</button>
                    </form>
                </motion.div>
            </div>
        )}
          ...
          <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 md:p-10 bg-background/95 backdrop-blur-xl">
            <motion.div initial={{ opacity: 0, scale: 0.98, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98, y: 20 }} className="bg-card border border-border rounded-3xl w-full max-w-4xl max-h-[80vh] flex flex-col shadow-2xl overflow-hidden my-auto">
              <div className="p-6 md:p-8 border-b border-border bg-background/50 flex justify-between items-center">
                <h2 className="text-xl font-black text-text uppercase tracking-tighter italic">Participant List</h2>
              ...
              <div className="flex-grow overflow-y-auto p-6 md:p-8 custom-scrollbar">
                {registrations.length > 0 ? (
                  ...
                  <p className="text-center py-16 text-text/30 font-black uppercase tracking-widest text-sm">No registrations yet.</p>
                )}
              </div>
            </motion.div>
          </div>
